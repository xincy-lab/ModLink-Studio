...
- `0.3.2rc2` 在 `0.3.1` 稳定性修订基础上补齐 replay seek 与录制导出体验
...
`0.3.2rc2` 继续以主宿主包 `modlink-studio` 作为公开安装入口。外部 driver 插件通常应该在自己的独立项目里开发，并把 `modlink-studio` 作为依赖；插件代码仍然从 `modlink_sdk` import SDK 类型，因为这个模块随 `modlink-studio` 分发。
